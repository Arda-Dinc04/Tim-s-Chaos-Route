...
function SingleStats({ route }: { route: RouteObject }) {
  return (
    <div className="stats-grid">
      ...
      <div className="stat-box">
        ...
        <p className="mt-1 text-lg font-black">{route.estimated_minutes.toFixed(1)} min</p>
      </div>
    </div>
  );
}

function CompareStats({ route }: { route: CompareRoute }) {
  return (
    <div className="space-y-2">
      <div className="grid grid-cols-[1fr_auto_auto] items-center gap-2 rounded-md bg-neutral-950 p-3 text-white">
        ...
        <span className="text-sm font-bold">{route.legal.estimated_minutes.toFixed(1)} min</span>
      </div>
      <div className="grid grid-cols-[1fr_auto_auto] items-center gap-2 rounded-md bg-[#fff1dc] p-3 text-[#7d2719]">
        ...
        <span className="text-sm font-bold">
          {route.shortcut.estimated_minutes.toFixed(1)} min
        </span>
      </div>
      <p className="text-sm font-bold">
        Saved: {formatMeters(Math.max(0, route.saved_distance_meters))} /{" "}
        {Math.max(0, route.saved_minutes).toFixed(1)} min
      </p>
    </div>
  );
}
  ...
  useEffect(() => {
    ...
    const debounce = window.setTimeout(async () => {
      ...
      try {
        ...
        const response = await fetch(`${API_URL}/geocode?${params.toString()}`, {
      ...
      <section className="control-card">
        <div className="mb-4 flex items-start justify-between gap-4">
          ...
          <button
            ...
            className="grid h-10 w-10 shrink-0 place-items-center rounded-md bg-neutral-950 text-white transition hover:bg-neutral-700"
            onClick={clearRoute}
            type="button"
          >
            <RotateCcw size={18} />
        ...
        <div className="mb-3 grid grid-cols-2 gap-3">
          <label className="block">
            ...
            <select
              className="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 font-bold text-neutral-950 outline-none ring-0 transition focus:border-neutral-950"
        ...
        <div className="space-y-3">
          ...
          {error && (
            <p className="rounded-md bg-red-50 p-3 text-sm font-bold text-red-800">
              {error}
            </p>
          )}
          ...
          {route &&
            (isCompareRoute(route) ? (
              <CompareStats route={route} />
            ) : (
              <SingleStats route={route} />
            ))}
